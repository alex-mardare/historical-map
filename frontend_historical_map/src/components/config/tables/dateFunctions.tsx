...
function convertDateToString(date: string | null): Date | undefined {
...
function dateColumnSort(
  firstObjectDate: HistoricalDateObject,
  secondObjectDate: HistoricalDateObject,
  firstDate: Date | undefined,
  secondDate: Date | undefined
) {
  // For negative dates the order is reversed
  const isFirstDateNegative =
    firstObjectDate.date && firstObjectDate.date[0] === '-'
  const isSecondDateNegative =
    secondObjectDate.date && secondObjectDate.date[0] === '-'
...
export { convertDateToString, dateColumnSort, extractDateFromString }
