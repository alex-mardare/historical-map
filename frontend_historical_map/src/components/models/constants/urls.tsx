const DEV_BASE_URL = 'http://localhost:8000'

const EVENT_CATEGORIES_SECTION = 'event-categories'
const HISTORICAL_EVENTS_SECTION = 'events'
const HISTORICAL_FIGURE_ROLES_SECTION = 'figure-roles'
const HISTORICAL_FIGURES_SECTION = 'figures'
const HISTORICAL_STATES_SECTION = 'historical-states'
const PRESENT_COUNTRIES_SECTION = 'present-countries'

const EVENT_CATEGORIES_ENDPOINT = '/api/event-categories/'
const HISTORICAL_EVENTS_ENDPOINT = '/api/events/'
const HISTORICAL_FIGURE_ROLES_ENDPOINT = '/api/figure-roles/'
const HISTORICAL_FIGURES_ENDPOINT = '/api/figures/'
const HISTORICAL_STATES_ENDPOINT = '/api/historical-states/'
const PRESENT_COUNTRIES_ENDPOINT = '/api/present-countries/'
const TOKEN_ENDPOINT = '/api/token/'
const TOKEN_REFRESH_ENDPOINT = '/api/token/refresh/'

export {
  DEV_BASE_URL,
  EVENT_CATEGORIES_ENDPOINT,
  EVENT_CATEGORIES_SECTION,
  HISTORICAL_EVENTS_ENDPOINT,
  HISTORICAL_EVENTS_SECTION,
  HISTORICAL_FIGURE_ROLES_ENDPOINT,
  HISTORICAL_FIGURE_ROLES_SECTION,
  HISTORICAL_FIGURES_ENDPOINT,
  HISTORICAL_FIGURES_SECTION,
  HISTORICAL_STATES_ENDPOINT,
  HISTORICAL_STATES_SECTION,
  PRESENT_COUNTRIES_ENDPOINT,
  PRESENT_COUNTRIES_SECTION,
  TOKEN_ENDPOINT,
  TOKEN_REFRESH_ENDPOINT
}
